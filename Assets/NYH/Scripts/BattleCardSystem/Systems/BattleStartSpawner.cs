namespace NYH.BattleCardSystem
{
    ...
    public class BattleStartSpawner : MonoBehaviour
    {
        ...
        // 예: RockWarrior count=5, baseHP=10 → 1기 스폰, maxHP=50
        ...
        // playerRoster가 null이면 fallback을 시도합니다.
        public List<BattleUnit> SpawnAll(BattleUnitRoster playerRoster, BattleUnitRoster enemyRoster)
        {
            ...
            if (hasRoster)
            {
                ...
                {
                    spawned.AddRange(SpawnFromRoster(playerRoster));
                }

                if (enemyRoster != null)
                {
                    spawned.AddRange(SpawnFromRoster(enemyRoster));
                }
            }
            else
            {
