...
public enum EnemyAgeGroupBase //나이별 상태
{
    Baby,
    Adult,
    Old
}
public enum EnemyCanMoveJobBase
{
    CanMoveJob, //병사임
    CannotMoveJob //농부, 상인 등등 이동 안하는 직업들
}
public enum EnemyLevelBase //시대별 상태임
{
    Level1,//구석기
    Level2, //신석기
    Level3 //철기
}
public enum EnemyUnitTypeBase
{
    //기본 생산 직업
    Farmer,
    Shoper,
...
public class EnemyUnitBase : MonoBehaviour
{
    ...
    [SerializeField] protected EnemyAgeGroupBase enemyAgeGroup;
    [SerializeField] protected float naturalDeathChance;

    [Header("직업에 따른 움직임 가능 여부")]
    [SerializeField] protected EnemyCanMoveJobBase enemyCanMoveJobBase;

    [Header("현 직업의 시대")]
    [SerializeField] protected EnemyLevelBase enemyLevelBase;
    ...
    void UnitAppear() //유닛이 소환되면 초기화 시킴.
    {
        ...
        enemyAgeGroup = EnemyAgeGroupBase.Baby; //나이 그룹도 아기부터 시작
        naturalDeathChance = 0f; //자연사망 확률도 0으로 시작
        enemyLevelBase = EnemyLevelBase.Level1; //시대도 구석기부터 시작
        enemyUnitID = 0; //소속 문명은 0번.
        attackPower = 1;
        anime = GetComponent<Animator>();
        switch (enemyCanMoveJobBase)
        {
            case EnemyCanMoveJobBase.CanMoveJob:
                //병사움직임
                StartMoveLoop();
                break;
            case EnemyCanMoveJobBase.CannotMoveJob:
                //농부, 상인 등등 이동 안하는 직업
                break;
            default:
                Debug.LogError("개버그임.");
    ...
    protected virtual void ChangeAgeGroup() //나이 그룹 변경
    {
        ...
        {
            case < 3:
                enemyAgeGroup = EnemyAgeGroupBase.Baby;
                break;
            case < 15:
                enemyAgeGroup = EnemyAgeGroupBase.Adult;
                break;
            case >= 15:
                enemyAgeGroup = EnemyAgeGroupBase.Old;
                break;
        }
    }
    ...
    {
        switch (enemyAgeGroup)
        {
            case EnemyAgeGroupBase.Baby:
                naturalDeathChance = 0f; //아기는 안죽어
                break;
            case EnemyAgeGroupBase.Adult:
                naturalDeathChance = 0.1f; //사람이 죽을 확률은 10%
                break;
            case EnemyAgeGroupBase.Old:
                naturalDeathChance = 0.7f + (0.05f * (age - 14)); //늙으면 죽을 확률이 70% 그 후로 확률업
                break;
        }
    }
    //=========================사망 시 풀 반환 처리를 위한 곳========================================
    private IEnemyPool enemyPool; //자신이 속한 풀 참조

    public void SetOwnerPool(IEnemyPool pool) //GetHuman에서 자신을 불러온 풀을 참조하는거임 그래야 원래 풀로 돌아갈 수 있음
    {
        enemyPool = pool; //풀 참조
    }
    protected virtual void Dead() //사망 처리
    {
        ...
        enemyPool?.ReturnHuman(gameObject); //pool이 null인지 체크 후 객체 풀로 반환하여 재사용
    }
    //======================== 가만히 있는 유닛 랜덤 이동 처리 =================================
