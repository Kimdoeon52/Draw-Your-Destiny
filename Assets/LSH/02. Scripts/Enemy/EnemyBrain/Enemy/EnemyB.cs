...
{//경제트리 Enemy
    ...
    {
        AddActionCase(EnemyAction.Building, 30); //건물35프로
        AddActionCase(EnemyAction.GetGold, 40); //골드 40프로
        AddActionCase(EnemyAction.TryOccupy, 10); //영지 5프로
    ...
    protected override void UpdateActionCases() //적 행동 확률 업데이트하는 함수임. 예를 들어 레벨업하면 건물 짓는 행동 확률이 올라가는 식으로.
    {
        ...
        if (enemyLevel == 2)
        {
            ...
            actionCases[2].weight = 20; //영지 점령 시도 확률 10%
            actionCases[3].weight = 0;
        }
        else if (enemyLevel == 3) //공격적으로 변함
        {
            actionCases[0].weight = 70; //건물 짓기 확률 60%
