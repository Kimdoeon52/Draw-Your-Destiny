...
public class WizzardBarracksBehaviour : UnitProducerBehaviour
{
    ...
    {
        // 연구 포인트 200 미만이면 tick을 올리지 않음
        if (ResourceManager.Instance == null || ResourceManager.Instance.Research < RequiredResearch)
        {
            Debug.Log($"[WizzardBarracks] 연구 포인트 부족 ({ResourceManager.Instance?.Research ?? 0}/{RequiredResearch}) — 생산 대기");
            return;
        }
